...
def main():
    ...
    flow = NiceFlow(input_dim=1536, n_layers=4, n_couplings=4,
                    hidden_dim=512)  # pierwotnie n_layers = args.layers jaki hidden dim?
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    optimizer = optim.Adam(flow.parameters(), lr=lr)

    for j in range(tra):
        flow.train() #czy potrzebne?
        data = train_replay.dataset(1, 1)
        it = iter(data)
        x = next(it)

        embed = agnt.wm.encoder(x)
        embed = np.array(embed)
        embed = torch.Tensor(embed)
        embed = embed.to(device)

        #features = get_features(data/x)
        ...
        for i in range(num_features):
            if features[i] == 0:
                current_mean.append(-1)
            else:
                current_mean.append(1)

        current_mean = torch.Tensor(current_mean)
        current_sigma = torch.Tensor([1])  # jakas stala, albo zalezna od p_i dla imbalanced datasets
        ...
        c_dist = Normal(current_mean[0].to(device),
                        current_sigma.to(device))  # czy może być wielowymiarowy rozkład normalny?
        ...
        print(loss, "llllllllllllllooooooossssssssssssss")
        (-loss).backward()
        nn.utils.clip_grad_norm_(flow.parameters(), 1.0)
        optimizer.step()
        print(c, "ccccccccccccccccccccccccccccccccccccc")
